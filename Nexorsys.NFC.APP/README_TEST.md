# 🧪 Guide de Test — Pinede NFC Kiosque

> **Document rédigé par Ali HAMIDY — Clinique La Pinède**
> Version Production v1.5.1-MIE | Mis à jour en Mai 2026

Ce document regroupe **tout ce dont vous avez besoin** pour tester l'application
Pinede NFC Kiosque, que vous disposiez ou non d'un lecteur NFC physique.
...
| Dossier de travail | `Publish_Portable\` ou `C:\Program Files\Pinede NFC Kiosque\` |

### Lancement de l'application

**Option A — Distribution unifiée (recommandé pour les tests client) :**
```
PinedeNFC_Setup\PinedeIdentityTerminal.exe
```
*(Le dossier `PinedeNFC_Setup` contient l'assistant graphique d'installation, l'application autonome, le modèle de configuration réseau, et le désinstallateur en un clic).*
...
| # | Action | Résultat attendu |
|---|---|---|
| T21 | Saisir le secret injecté par le déploiement | Navigation vers le Panneau d'Administration |
...
### 5.2. Onglet NFC
*(Permet de surveiller et de diagnostiquer la présence de lecteurs NFC physiques).*
...
```powershell
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" | Select-Object "Pinede*"
```
...
|---|---|---|
| "Badge détecté avant AD" | INFO | Scan badge physique |
...
- Veuillez vérifier que le service Windows **"Carte à puce"** est démarré dans `services.msc`.
...
**✅ Application validée pour la production quand tous les tests critiques (T01, T05, T21, T32) sont cochés.**

---

*Document rédigé par Ali HAMIDY — Clinique La Pinède — Version Production v1.5.1-MIE*
