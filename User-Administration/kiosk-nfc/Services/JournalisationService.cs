...
namespace Nexorsys.NFC.APP.Services
{
    public class JournalisationService : IJournalisationService
    {
        ...
        public JournalisationService(ApiClientService apiClient)
        {
        ...
        public void EnregistrerEvenement(EvenementJournal evenement)
        {
            lock (_verrouEcriture)
            {
                try
                {
                    ...
                    // PGSSI-S: Traçabilité en temps réel déportée sur le serveur central
                    _ = _apiClient.TraceAsync(evenement.Niveau, evenement.Message ?? string.Empty);
                }
                catch
                {
